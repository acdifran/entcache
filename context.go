...
// Use returns a new Context that tells the Driver
// to use the cache on Query and will overwrite previous Skip and Evict.
//
//	client.T.Query().All(entcache.Use(ctx))
func Use(ctx context.Context) context.Context {
	c, ok := ctx.Value(ctxOptionsKey).(*ctxOptions)
	if !ok {
		return context.WithValue(ctx, ctxOptionsKey, &ctxOptions{skip: false, evict: false})
	}
	c.skip = false
	c.evict = false
	return ctx
